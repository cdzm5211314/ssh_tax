...
public class ExcelUtil {
	...
	/**
	 * 
	 * @方法的名称: exportUserExcel
	 * @Description: 导出用户的所有列表到Excel
	 * @Author: chenD
	 * @CreateDate: 2017-10-30 下午5:03:32
	 * @param userList
	 *            用户列表
	 * @param outputStream
	 *            输出流
		...
		try {
			...
			if (userList != null) {
				for (int j = 0; j < userList.size(); j++) {
					...
					cell22.setCellValue(userList.get(j).getAccount());
					HSSFCell cell33 = row.createCell(2);
					cell33.setCellValue(userList.get(j).getDept());
					HSSFCell cell44 = row.createCell(3);
					cell44.setCellValue(userList.get(j).isGender() ? "男" : "女");
					HSSFCell cell55 = row.createCell(4);
					cell55.setCellValue(userList.get(j).getEmail());
				}
			}
	...
	/**
	 * 
	 * @方法的名称: createCellStyle
	 * @Description: 创建单元格样式
	 * @Author: chenD
	 * @CreateDate: 2017-10-30 下午3:55:50
	 * @param workbook
	 *            工作博
	 * @param fontSize
	 *            字体大小
